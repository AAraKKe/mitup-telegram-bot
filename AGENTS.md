...
- register_conbersation_handler: this is a special type of handlers for handling long conversations with different states depending on what the user answers. These receive a list of other handlers that have different roles:
...
- regisger_inline_handler: this is a handler for inline queries.
...
- When explaining something technical, be sure to add code blocks that can be expanded with a heading explainign what is hidden. Only make it expandable if the content is more than 10 lines of code.
- Try to use emojis for conveying emotions. Mostly on headers and never withing paragraphs. Use the twimoji emojis instead of simple emojis, e.g. :pen: or :check_mark:
- Whena adding examples, do not assume meeting only refers to business meetings. These refer to all types of meetings, mostly with friends. Add examples that showcase different situations.
...
Documenation files, under the `docs` folder, are served with mkdocs. The mkdocs config file in the root of the project is used to handle mkdocs configuraiton.
...
When creating a heading (either through standard markdown heading or in bold to simulate a heading) allays have the heading surounded by blank lines to ensure that formatting is correct.
...
The database interaction is mocked through the `mock_session` fixture available to all tests and the api interaction is mocked with the `api` fixture which is avilable throughe very module conftest.
...
When we need to use the Telegra.Update object on a given test, we can provide this through a parameter of type UpdateRequest (under tests.helpers.fixtures). This UpdateRequest object is used as an indirect parameter to the `update` fixture
...
- handlers: this is where most of the logic of the bot is. We use [Telegram Python Bot](mdc:https:/docs.python-telegram-bot.org/en/stable/index.html) (PTB) as the sdk to develop the bot and all the bot behavior is defined through handlers.
  - Handlers are organized in sub modules semantically defined. We have submodules roughly referencing each part of the bot features or areas.
  - Each sub module int he handlers module contains 2 main modules: `enums` and `entry`. These referece all enums used to identify handlers or conversations and the callback that is the entry point for that feature.
  - These do no have any runtime implication and is just a way of being able to quickly identify where a piece of code can be.
- The labmdas module contains the code that is run as a lambda function in AWS
...
- utils: this contains several utilities used around the bot. The most important ones are `messages` and `callbacks`. Messages ocntain the english verison of any text that appears in the bot and `callbacks` contains general callbacs that represent teh callback data of a request to PTB
...
- callbac_data contains the cnetralized definition of how callback data is handled in a request. All callbacks in the utils.callbacks module are instance of this callback data
- callback_id contains de definition of a callback id, used to identify each handler.
...
- guards are a et of methods that are used to validate input received by a handler
- timezone_api contains the logic to interact witht he google timezone api
- translations defines the transaltions engine, a wrapper around gettext to transalte text
