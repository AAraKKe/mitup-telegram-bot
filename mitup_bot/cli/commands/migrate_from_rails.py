...
def cli(
    ...
    report = run_migration_pipeline(rails_url, archive_s3_uri, batch_size, selected_phases, mode, metrics, reporter)
    asyncio.run(metrics.flush())
...
def run_migration_pipeline(
    ...
    report: dict[str, Any] = {}
    try:
        with RailsReader(rails_url, batch_size=batch_size) as reader, db.begin() as session:
