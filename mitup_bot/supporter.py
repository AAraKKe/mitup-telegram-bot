...
LEVEL_ORDER: tuple[SupporterLevel, ...] = (
    SupporterLevel.NONE,
    SupporterLevel.SUPPORTER,
    SupporterLevel.PATRON,
    SupporterLevel.ORGANIZER,
)
...
def is_supporter(level: SupporterLevel) -> bool:
    ...
    return meets(level, SupporterLevel.SUPPORTER)
...
def level_for_amount(cents: int, config: PatreonConfig) -> SupporterLevel:
    ...
    it reaches. An active member below the lowest threshold still counts as SUPPORTER — an active
    ...
    if cents >= config.organizer_min_cents:
        return SupporterLevel.ORGANIZER
    if cents >= config.patron_min_cents:
        return SupporterLevel.PATRON
...
def active_meetings_cap(level: SupporterLevel) -> int | None:
    ...
    match level:
        case SupporterLevel.NONE | SupporterLevel.SUPPORTER:
...
def participant_capacity(level: SupporterLevel) -> int | None:
    ...
    match level:
        ...
        case SupporterLevel.PATRON | SupporterLevel.ORGANIZER:
            return None
        case _ as unreachable:
            assert_never(unreachable)
...
def badge(level: SupporterLevel) -> str | None:
    ...
    match level:
        ...
        case SupporterLevel.SUPPORTER:
            return Emojis.SUPPORTER.value
        case SupporterLevel.PATRON:
            return Emojis.PATRON.value
        case SupporterLevel.ORGANIZER:
            return Emojis.ORGANIZER.value
        case _ as unreachable:
            assert_never(unreachable)
