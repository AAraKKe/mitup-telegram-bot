...
> **Type checker note:** Call sites of `@with_async_session` functions trigger a false-positive `missing-argument` from `ty` due to missing `Concatenate` support. Suppress with `# ty: ignore[missing-argument]` and a tracking issue URL. See the `type-checking` reference skill.
...
4. Register the handler in `tests/test_failure_modes.py` if it uses guards (`current_user`, `meeting_accessible`, `valid_callback_data`, etc.) — see `tests/CLAUDE.md` for details.
