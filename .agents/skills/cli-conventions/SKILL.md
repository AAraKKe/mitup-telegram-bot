...
description: CLI and recurrent-event conventions for mitup_bot. Auto-load when writing, editing, or reviewing the app CLI entry modules (bot_cli.py, events_cli.py) or recurrent-event jobs under apps/events/mitup_bot/events/.
...
| App | Entry module | Console script | Command(s) |
|---|---|---|---|
...
<critical_rules>
  <rule>These CLIs are for service entry points only — the commands the production image must run (launching the bot, running recurrent events). Dev/CI tooling (deploying, managing locales, validating migrations) lives in the `mb` CLI, not here.</rule>
