...
message was: it deletes that message and hands the view to the background worker under
`screen_replacement_strategy`, whose hold lets the client's dissolve animation play out before the
view lands without holding the request, and edits instead once `message_is_deletable` reads the message as past the 48 hour
