...
In both modes the runtime ends with `uvicorn.Server(uvicorn.Config(app=fastapi_app, host="0.0.0.0", port=config.bot.listen_port, workers=1, log_config=None, access_log=False, lifespan="on")).run()`. The lifespan owns the PTB lifecycle in full — startup and shutdown happen inside the context manager.
...
- `app.py` — the FastAPI factory `create_app(...)`, the webhook and polling lifespan builders (both of which run the unrouted-request reporter around their `yield`), the `run_shutdown_step` helper that isolates each shutdown step's failure, and the one `http` middleware that publishes the metrics client, counts an unrouted request and flushes.
- `access_log.py` — the in-process `UnroutedRequests` count and the interval reporter that publishes it, which is what stands in for uvicorn's access log.
...
- **`uvicorn` must be invoked with `access_log=False`.** The endpoint is published by Certificate Transparency and fronted by a layer-4 NLB that cannot filter paths, so internet-wide vulnerability scanners reach uvicorn directly: one host sent 21k requests across 11k invented paths in ninety minutes, and one line each is the whole log stream. `access_log=False` empties the `uvicorn.access` logger's handlers and clears its `propagate`, so the protocol's `hasHandlers()` check is false and it never even builds the record — strictly better than pinning the level in `configure_library_levels`, which still formats a record per request before dropping it. `uvicorn.error` is untouched: real server errors must still reach the stream.
- **Nothing is written per unrouted request — not a line, not a metric, and not a flush.** An EMF emission *is* a log line, so swapping an access line for a per-request record saves nothing; and `MetricsClient.flush()` walks every logger `EmfBackend` has cached and writes a document for each, so flushing a request that emitted nothing is the same cost again. The middleware therefore flushes only when a route matched — an unrouted request never reaches an endpoint, so nothing can have emitted during it. `access_log.py` keeps an in-process `UnroutedRequests` accounting that the middleware feeds, and a reporter task started from the lifespan publishes it every `UNROUTED_REPORT_INTERVAL_SECONDS` and resets: one `MetricKey.UNROUTED_REQUEST` sample plus one `Unrouted request sweep summary` WARNING line. **A quiet interval must emit nothing at all** — a zero sample per interval reinstates the volume the aggregation removes, and absence is the correct reading here ("nobody probed us"), unlike a gauge such as `NEW_MEMBERS` where the dense series is the point. The series is dimensionless on purpose: one billed series and one cached `EmfBackend` logger instead of one per facet value.
- **A request a route served is not accounted for by the middleware.** The endpoint that handled it already narrates itself (`Telegram update received`, the update trace, the Patreon endpoints' own lines). A generic served-request line on top is duplication, so the middleware records nothing for a matched route and computes no timing for one.
- **The request path never reaches the metric plane, and reaches the log plane only through the bounded per-interval summary.** Path and method are caller-controlled text with unbounded cardinality, so neither may ever land on a metric dimension, an EMF property, or any per-request log line. The one sanctioned outlet is the `Unrouted request sweep summary` line `access_log.py` writes at most once per interval: methods and paths truncated, at most `UNROUTED_TRACKED_TARGETS` distinct targets counted (the overflow is a single tally), only the top `UNROUTED_REPORTED_TARGETS` named — bounds that hold whatever a scanner sends, which is what makes the outlet safe. **The query string never reaches the accounting** — the middleware passes `request.url.path`, and a near-miss on a real route (a trailing-slash Patreon callback) could carry a live OAuth code in its query. Never widen this to routed requests, headers, or bodies, and never turn the summary's contents into per-request records.
- **`create_app` installs exactly one `http` middleware.** Each `@app.middleware("http")` wraps the app in a full `BaseHTTPMiddleware` with its own task group and stream pair, per request. The `bound_metrics_client` publication, the unrouted count and the metrics flush all belong in that single pass — add to it rather than stacking a sibling.
