...
- **The request path never reaches a log line, a metric dimension, or an EMF property.** It is caller-controlled text with unbounded cardinality. Whether the router matched anything (`isinstance(request.scope.get("route"), Route)`, read after `call_next`) is the only thing the middleware may learn from the request target. The HTTP method is caller-controlled too and is not recorded anywhere.
