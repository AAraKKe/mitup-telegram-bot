...
- **Relationship-level selectin does not cascade through a load-path cycle.** When the eager-load path reaches a mapper already on it, SQLAlchemy stops implicit eager loading at that hop. A user-rooted load cycles `User -> Meetup -> JoinedUsers -> User`, so each meeting's `owner` and its participants' `user`/`invited_by` behind `meetups`/`joined_links` are dropped even though those relationships are `lazy="selectin"` on their mapper. Loads rooted at a user that render participant lists must therefore **spell out the full loader chains** to the leaves the view reads — see `user_collection_loaders` in `models/users.py` and the export statements in `handlers/privacy/data_export.py`. Meetup-rooted loads (`Meetup.by_id`) repeat no mapper and are unaffected. `User.by_tg_user_id` gates the cost: its default loads the one-hop collections plus `joined_links -> meetup` (what the list screens need), and only `load_participants=True` adds the deep participant/owner chains — pass it (via `guards.current_user(..., load_participants=True)` or the classmethod directly) only from handlers that render a full meeting card straight off `user.meetups`/`user.joined_links` rather than re-loading through `Meetup.by_id`. The inline query is currently the sole such caller.
