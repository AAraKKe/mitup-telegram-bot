...
<critical_rules>
  ...
  <rule>Every new process entry point (bot runtime, CLI task, Lambda handler) must call `configure_logging(env, component, level)` once before any logging, then bind its own invocation fields via `structlog.contextvars.bound_contextvars(...)`. See `handlers/registry.py`, `apps/events/mitup_bot/events/service.py`, and `apps/lambda-migrations/mitup_bot/lambdas/migrations.py` for the canonical pattern.</rule>
  <rule>Reserved identity keys — one id space per key, never mixed: `tg_user_id` for a Telegram user id, `user_id` for an internal DB primary key (`user.db_id` / a `users.id` FK), `chat_id` for a Telegram chat id. Pick the key from the value's source, not from the local variable name. Log filtering must key on these structured fields, never on the event message text.</rule>
</critical_rules>
