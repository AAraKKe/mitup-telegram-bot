...
class _FakeConstraintError(Exception):
    ...
    Carries the psycopg-style ``diag.constraint_name`` that ``flush_new_participant`` inspects. When no
...
def integrity_error(constraint_name: str | None) -> IntegrityError:
    ...
    ``flush_new_participant`` decides whether to swallow a clash by inspecting
