...
def assert_locked_meetup_select(mock_session: MockDbSession):
    ...
    Deliberately tolerant of dialect/formatting details: every executed meetups SELECT that loads
    rows must carry FOR UPDATE. Counting queries are left out, since an aggregate locks nothing.
    Flows that legitimately pre-read the meeting unlocked (e.g. earlier conversation steps) should
    reset ``mock_session.exec`` before the mutating step so only its queries remain.
    """
    meetup_selects = [
        query
        for query in mock_session.queries_executed
        if query.lower().startswith("select") and "from meetups" in query.lower() and "count(" not in query.lower()
    ]
