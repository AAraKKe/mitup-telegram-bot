...
async def test_flush_new_participant_returns_true_when_flush_succeeds(mock_session: MockDbSession):
    assert await flush_new_participant(mock_session, mock.MagicMock(), mock.MagicMock()) is True
    mock_session.assert_flushed()


async def test_flush_new_participant_swallows_uniqueness_clash(mock_session: MockDbSession):
    ...
    assert await flush_new_participant(mock_session, mock.MagicMock(), mock.MagicMock()) is False
...
async def test_flush_new_participant_reraises_other_integrity_errors(
    ...
    with pytest.raises(IntegrityError):
        await flush_new_participant(mock_session, mock.MagicMock(), mock.MagicMock())
