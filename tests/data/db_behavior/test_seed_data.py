...
async def test_user_persisted(db_session: AsyncSession, seed_user: User):
    loaded = (await db_session.exec(select(User).where(User.tg_user_id == 999_001))).one()
    assert loaded.first_name == "Seed User One"
    assert loaded.id is not None


async def test_settings_created_for_user(db_session: AsyncSession, seed_user: User):
...
async def test_meetup_persisted(db_session: AsyncSession, seed_meetup: Meetup):
...
async def test_joined_link_persisted(db_session: AsyncSession, seed_joined_link: JoinedUsers):
...
async def test_meetup_location_defaults_empty(db_session: AsyncSession, seed_meetup: Meetup):
    loaded = (await db_session.exec(select(Meetup).where(Meetup.id == seed_meetup.id))).one()
