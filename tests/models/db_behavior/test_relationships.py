...
async def test_user_has_settings_relationship(db_session: AsyncSession, seed_user: User) -> None:
...
async def test_user_has_meetups_relationship(db_session: AsyncSession, seed_user: User, seed_meetup: Meetup) -> None:
    loaded = (await db_session.exec(select(User).where(User.id == seed_user.id))).one()
...
async def test_meetup_has_owner_relationship(db_session: AsyncSession, seed_meetup: Meetup, seed_user: User) -> None:
...
async def test_joined_link_invited_by_is_none(db_session: AsyncSession, seed_joined_link: JoinedUsers) -> None:
    loaded = (await db_session.exec(select(JoinedUsers).where(JoinedUsers.id == seed_joined_link.id))).one()
