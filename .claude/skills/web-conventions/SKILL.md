...
description: HTTP/web layer conventions for mitup_bot — the FastAPI app factory `create_app()`, the uvicorn server invocation inside `MitupRuntime.run()`, the `POST /telegram` webhook endpoint, the `@asynccontextmanager` lifespan that owns PTB's `initialize`/`start`/`set_webhook`/`stop`/`shutdown` sequence, secret-token validation, and FastAPI dependency injection with `Annotated[T, Depends(...)]`. Use this skill whenever the work touches `mitup_bot/web/`, the FastAPI/uvicorn/lifespan parts of `mitup_bot/app.py`, the PTB webhook lifecycle (`Application.builder().updater(None)`, `app.process_update`, `bot.set_webhook`), adding new HTTP routes, OAuth callbacks, the `/telegram` endpoint, or any question about how uvicorn serves the bot — even when the task description doesn't mention FastAPI by name. Includes the hard rules that prevent silent breakage (uvicorn `workers=1`, `log_config=None`, never pass `allowed_updates`, never return non-2xx from `/telegram`).
...
|---|---|---|---|
| `WEBHOOK` (prod) | `.updater(None)` — disables PTB's internal updater | `await initialize()` → `await start()` → `await bot.set_webhook(...)` | Updates arrive at `POST /telegram` and are fed via `await app.process_update(update)` |
...
Telegram → NLB (TCP, TLS terminates) → ECS container :80 → uvicorn → FastAPI → telegram_webhook()
  ...
  4. await ptb_app.process_update(update) — exceptions logged but swallowed; we still return 204.
  5. Return 204 No Content. Always.
```
...
- **The `/telegram` endpoint must NEVER return non-2xx for application errors.** Telegram retries on any non-2xx. Returning 5xx on a poison-pill update creates a retry storm and floods the bot. Wrap every parse/process step in `try/except`, log, and return 204.
...
- **PTB `initialize()` must complete before any `process_update()` call.** `Update.de_json` needs the bot, and `process_update` needs the application started. uvicorn's `lifespan="on"` enforces this — never accept HTTP traffic before the lifespan context is entered.
