...
<core_directives>
  ...
  <rule>Every mention of a bot button in prose MUST use the `.button-like` chip — no exceptions, no plain bold, no monospace. Look up the exact text and emoji in `ButtonMessages` in `libs/telegram/mitup_bot/utils/messages.py` and apply the recipe from `docs-style`.</rule>
