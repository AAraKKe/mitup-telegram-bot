...
**Every agent owns conventions end-to-end** — no separate review agent runs after it. Each brief must state this explicitly and name the skills to load, including the recurring hard rules (module-level names are public; a comment or docstring exists only to say what the code cannot say by existing, one or two sentences, no history or rationale essays; names are plain English). The orchestrator independently verifies the agent's actual diff after every report — read the changed files, re-run the key proofs, scan comments for history prose — rather than trusting the report or outsourcing the check.

**Always include the current working tree's absolute root in subagent prompts.** Compute it with `git rev-parse --show-toplevel` and tell the specialist to root every file path under it — don't pattern-match on directory names, since worktree layouts vary (`.claude/worktrees/<name>/`, `wt/<name>/`, anywhere else). Skills reference paths as relative (`apps/bot/mitup_bot/handlers/`, `tests/`); subagents otherwise resolve them against whatever directory they happen to start in, which is usually the main checkout. This matters most when you're working in a `git worktree` other than the main checkout, because the main checkout typically has unrelated in-flight work on a different branch and writes there silently collide with it.
...
**Before opening any MR, review the full branch diff against the repo's default branch** (`git diff "$(git symbolic-ref --short refs/remotes/origin/HEAD)"...HEAD`, typically `origin/main`) regardless of who wrote the code — run the full validation gate plus plain `ruff check .`, and check the diff against the hard conventions above; manually-edited code skips the per-agent ownership pass and routinely slips otherwise.
