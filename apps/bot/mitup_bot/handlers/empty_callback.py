...
async def callback_query_empty(update: Update, context: TMitupContext):
    ...
    Telegram accepts no inline button without callback data, so decorative keyboard cells,
    like the label cells of the calendar keyboards still sitting in old chats, carry `cb.EMPTY`.
    Binding it keeps those taps away from the unrouted-callback fallback, which reads unmatched
    data as a button shipped with no handler and fails the interaction.
